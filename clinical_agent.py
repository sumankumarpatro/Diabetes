...
class Symptom(BaseModel):
    name: str = Field(...)
    is_negated: bool = Field(..., description="True if symptom is denied/absent in the note.")

class ClinicalFeatures(BaseModel):
    age_group: str = Field(..., description="e.g. Adult, Pediatric, Geriatric")
    symptoms: List[Symptom] = Field(default_factory=list)
    medications: List[str] = Field(default_factory=list)
    medication_count: Optional[int] = Field(None, description="count from the note text")
    condition_status: str = Field("Unknown", description="Stable, Unstable, etc.")
    hospital_stay_days: Optional[int] = Field(None)
    glucose_status: str = Field("Unknown", description="Hyperglycemia / Hypoglycemia / Unknown")
